...
struct NoABBusca { /* structpara nó em árvore
                             binária */
  int info;
...
char Equilibrada(ABBusca *ab, int *N) {
  ...
  if (ab == NULL) {
    *N = 0;
    ...
    if (Equilibrada(ab->esq, &Ne) && Equilibrada(ab->dir, &Nd) &&
        (abs(Ne - Nd) <= 1)) {
      (*N) = Ne + Nd + 1;
...
int NivelNo(ABBusca *ab, int chave, int *nivel) {
  if (ab != NULL) {
    ...
    // se a chave é igual ao no atual
    if (ab->info == chave) {
      (*nivel) += 1;
      return 1;
    } else {
      ...
      if (ab->info > chave) {
        (*nivel) += 1;
        return (NivelNo(ab->esq, chave, nivel) + 1);
      } else {
        (*nivel) += 1;
        return (NivelNo(ab->dir, chave, nivel) + 1);
      }
    }
  }
  return (0);
...
int NosMaiores(ABBusca *ab, int valor, int *qtd) {
  if (ab != NULL) {
    if (ab->info > valor) {
      (*qtd) += (NumerodeNos(ab->dir) + 1);
      return NosMaiores(ab->esq, valor, qtd);
    }
    if (ab->info == valor) {
      (*qtd) += (NumerodeNos(ab->dir));
    }
  }
  return 0;
}